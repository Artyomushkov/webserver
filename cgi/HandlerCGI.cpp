...
std::vector<std::string> HandlerCGI::init_env(Connect *conn) {
	...
	res.push_back(form_env_string("PATH_TRANSLATED", conn->location->getRoot() +
	conn->head.get("uri")));
...
conn) {
	...
	close(fdOut[1]);
	char* args[3];
	std::string script = conn->location->getRoot() +
...
ContentFile HandlerCGI::handleCGI(Connect* conn) {
	...
	if (conn->unChunked.len())
		write(fdIn[1], conn->unChunked.get_content(), conn->unChunked.len());
	else
	...
	if (proc == 0) {
		forkCGI(fdIn, fdOut, env, conn);
	}
	int status;
	waitpid(proc, &status, 0);
	if (WEXITSTATUS(status)) {
		res.push_back("Error in fork");
		return res;
	}
	char buffer[30];
	size_t ret = 30;
